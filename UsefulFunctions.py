...
def get_dataframe_laser(inputfiles, whichstats, channels=[1,2,3,4], rmscut=1.5, residualcut=5, p0=[(0,100,1,110,-100,100),(0,100,1,110,-100,100),(0,100,1,110,-100,100),(0,100,1,110,-100,100),(0,100,1,110,-100,100)],eventstart=1,verbose=False,show_residual=False,vieweventstart=1,viewevents=10):
    #error handling
    error = False
    if "list" not in str(type(inputfiles)):
        print("ERROR: inputfiles (first input) must be a list of strings.")
    ...
    for i in range(0, filenumber):                             #iterate through files
        ...
        for j in range(eventstart, len(current_file)):                  #iterate through events len(current_file)
            ...
            if verbose and (j >= vieweventstart) and (j <= viewevents+vieweventstart): #show the waveform fit line
                print(f"Event Number {j}")
                fig,ax = plt.subplots(1,len(channels),figsize=(8*len(channels),4))
                for i in range(len(channels)):
                    ax[i].set_title(f"Ch. {channels[i]}")
            ...
            for channel in channels:
                ...
                if totalrms < rmscut:
                    ...
                    if do_fit and (channel) == 1:
                        ...
                        while amp < 450:
                            i += 1
                            amp = (voltage[channel-1])[i]
                        amplitude_fit = get_amplitude_base(voltage[channel-1])
                        din[f'ch{channel}_P2P_fit'].append(amplitude_fit)
                        pulse_time_fit = time[i]
                        din[f'ch{channel}_time_fit'].append(pulse_time_fit)
                ...
                if verbose and (j >= vieweventstart) and (j <= viewevents+vieweventstart): #show the waveform fit line
                    ...
                    #plot the data, smooth if smoothing, fit if fitting
                    ax[findindex(channels,channel)].plot(time,voltage[channel-1],label="raw")
                    if do_smooth: ax[findindex(channels,channel)].plot(time,smoothvoltage,label="smooth_5", color='orange')
                    if channel != 1:
                        if do_fit: ax[findindex(channels,channel)].plot(ts,fits,label="fit",color="black")
                    if show_residual: ax[findindex(channels,channel)].plot(time,residual,label="residual")

                    #draw the P2P and time 
                    if do_raw: 
                        baseline_raw = np.min(voltage[channel-1][0:findindex(voltage[channel-1],np.max(voltage[channel-1]))+1])
                        ax[findindex(channels,channel)].vlines(pulse_time_raw, ymin=baseline_raw, ymax=amplitude_raw+baseline_raw, color='r', label="t_raw")
                        ax[findindex(channels,channel)].hlines(baseline_raw, xmin=pulse_time_raw-30, xmax=pulse_time_raw+30, color='r')
                        ax[findindex(channels,channel)].hlines(amplitude_raw+baseline_raw, xmin=pulse_time_raw-30, xmax=pulse_time_raw+30, color='r', label="A_raw")

                    if do_base: 
                        baseline_base = np.mean(voltage[channel-1][0:150])
                        ax[findindex(channels,channel)].vlines(pulse_time_base, ymin=baseline_base, ymax=amplitude_base+baseline_base, color='b',label="t_base")
                        ax[findindex(channels,channel)].hlines(baseline_base, xmin=pulse_time_base-30, xmax=pulse_time_base+30, color='b')
                        ax[findindex(channels,channel)].hlines(amplitude_base+baseline_base, xmin=pulse_time_base-30, xmax=pulse_time_base+30, color='b',label="A_base")

                    if do_smooth: 
                        baseline_smooth = np.mean(smoothvoltage[0:151])
                        ax[findindex(channels,channel)].vlines(pulse_time_smooth, ymin=baseline_smooth, ymax=amplitude_smooth+baseline_smooth, color='purple',label="t_smooth_5")
                        ax[findindex(channels,channel)].hlines(baseline_smooth, xmin=pulse_time_smooth-30, xmax=pulse_time_smooth+30, color='purple')
                        ax[findindex(channels,channel)].hlines(amplitude_smooth+baseline_smooth, xmin=pulse_time_smooth-30, xmax=pulse_time_smooth+30, color='purple', label="A_smooth")

                    if do_fit: 
                        ax[findindex(channels,channel)].vlines(pulse_time_fit, ymin=popt[0], ymax=popt[0]+amplitude_fit, color='g',label="t_fit")
                        ax[findindex(channels,channel)].hlines(popt[0], xmin=pulse_time_fit-30, xmax=pulse_time_fit+30, color='g')
                        ax[findindex(channels,channel)].hlines(amplitude_fit+popt[0], xmin=pulse_time_fit-30, xmax=pulse_time_fit+30, color='g',label="A_fit")
                    ax[findindex(channels,channel)].legend() 
